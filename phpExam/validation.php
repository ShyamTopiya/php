   ...
   foreach($_POST as $fieldName => $fieldValue) {
       switch($fieldName) {
           ...
           case 'lastName'     :   if(!preg_match('/^[a-zA-Z]*$/', $fieldValue))
                                   ...
                                   break;
           case 'password'     :   if($fieldValue !== $_POST['cpassword'])
                                       array_push($errList, $fieldName);
                                   else    
           ...
           case 'phoneNumber'  :   if(!preg_match('/[0-9]/', $fieldValue) || strlen($fieldValue) != 10)
                                   ...
                                   break;
            case 'information'  :   $preparedData['information'] = $fieldValue;
                                   break;
   ...
   foreach($_POST as $fieldName => $fieldValue) {
       switch($fieldName) {
           ...
           case 'metaTitle'     :    $preparedData['metaTitle'] = $fieldValue;
                                   break;
            case 'parentCategory'     :  $parentId = findParentKey($fieldValue); 
   ...
   foreach($_POST as $fieldName => $fieldValue) {
       switch($fieldName) {
           ...
           case 'URL'     :         $preparedData['url'] = $fieldValue;
                                   ...
                                   break;
            case 'parentCategory'     : if(is_array($fieldValue))
                                        $preparedData['parent_name']  = implode(",", $fieldValue);
                                        else
                                        $preparedData['parent_name'] = $fieldValue;
                                        break;
            case 'publishedAt'     :    $preparedData['published_at'] = $fieldValue;
                                       break;
